// ContextShift — You.com Content Script

(() => {
  function scrapeConversation() {
    const messages = [];

    const userBlocks = document.querySelectorAll('[class*="userMessage"], [class*="user-message"], [class*="query"]');
    const aiBlocks = document.querySelectorAll('[class*="aiMessage"], [class*="ai-message"], [class*="answer"], [class*="youchat-answer"]');
    ...
    if (userBlocks.length > 0 || aiBlocks.length > 0) {
      ...
      const container = document.querySelector('[class*="chat"], main');
      if (container) {
        const blocks = container.querySelectorAll('[class*="message"], [class*="turn"]');
        blocks.forEach((block, i) => {
          const classList = (block.className || '').toLowerCase();
          const isUser = classList.includes('user') || classList.includes('query');
          const role = isUser ? 'user' : i % 2 === 0 ? 'user' : 'assistant';
